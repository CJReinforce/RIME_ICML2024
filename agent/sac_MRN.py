...
class SACAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, cfg,
        ...
        # optimizers
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=cfg.actor_lr, betas=cfg.actor_betas)
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=cfg.critic_lr, betas=cfg.critic_betas)
        self.critic_old_optimizer = torch.optim.Adam(self.critic_old.parameters(), lr=cfg.critic_lr, betas=cfg.critic_betas)
        self.log_alpha_optimizer = torch.optim.Adam([self.log_alpha], lr=cfg.alpha_lr, betas=cfg.alpha_betas)

        # change mode
        self.train()
        self.critic_target.train()

    def update_critic_old(self):
        self.critic_old.load_state_dict(self.critic.state_dict())
    ...
    def load(self, model_dir, step):
        self.actor.load_state_dict(torch.load('%s/actor_%s.pt' % (model_dir, step), map_location=torch.device('cpu')))
        self.critic.load_state_dict(torch.load('%s/critic_%s.pt' % (model_dir, step), map_location=torch.device('cpu')))
        self.critic_target.load_state_dict(torch.load('%s/critic_target_%s.pt' % (model_dir, step), map_location=torch.device('cpu')))
